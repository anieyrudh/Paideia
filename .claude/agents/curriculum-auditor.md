...
Caller hands you a container directory: `<branch>/content/<subject>/concept-packages/<package-id>/`.

Read in this order:
1. `concept-package.yaml` — read `branch`, `subject`, `syllabus_ref`, `level`, `title`.
...
- Frontmatter `subject`, `branch`, `level` MUST be consistent with `concept-package.yaml`.
