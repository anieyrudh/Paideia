...
loss, and drag force. It is pure TypeScript and returns `KernelResult` values
...
- Public physical quantities use SI units: kg m^-3, Pa s, Pa, m, m^2, m^3,
  m^3 s^-1, m s^-1, m s^-2, N, and dimensionless ratios.
...
Darcy-Weisbach pipe losses, or quadratic drag. If the sim needs CFD visuals,
compose those visuals around this kernel's scalar claims rather than placing
the formulas in the container.
...
`fluidMechanicsTolerance.default` is rejected.
