...
describe("partial derivatives and gradient embed contract", () => {
  it("exposes load, state, score, resume, theme sync, and destroy", async () => {
    const api = createEmbed();
    const target = document.createElement("div");

    await api.load(target);
    expect(target.dataset.paideiaContainer).toBe("partial-derivatives-and-gradient");
    ...
    api.syncTheme({ colorScheme: "dark", accentColor: "#0f766e" });
