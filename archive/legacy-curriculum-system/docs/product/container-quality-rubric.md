...
| `simulation/` | Required for sim-worthy concepts; includes runtime, controls, presets, state labels, prediction checkpoint when declared, and an executable or contract test |
...
Text-only observation states are not product-quality simulations.
See `docs/quality/visual-simulation-standard.md` and
`docs/quality/visual-exemplar-gallery.md` for the enforceable live-simulation contract
...
| --- | --- |
| Prediction checkpoint | A compact checkpoint is visible, saves the learner's expectation and rationale, and never blocks the simulation |
| Direct manipulation | Sliders, drags, toggles, presets, or other inputs visibly change the model |
| Visual model | Observation state contains a real chart, plot, SVG diagram, canvas, 2D scene, 3D scene, or equivalent interactive visual artifact on first load |
...
| --- | --- | --- |
| P0 | Yes | Invalid container shape, missing prediction checkpoint, hidden or missing visual for sim-worthy concept, cross-branch import, GPL runtime code, uncited copied content |
...
- Screenshot or short recording of the live simulation state.
