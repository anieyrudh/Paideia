...
test.describe("a-level/physics/gravitational-fields/inverse-square-field-lab prediction-checkpoint", () => {
  test("prediction-checkpoint keeps field and potential readouts visible while saving reflection", async ({ page }) => {
    await mountSim(page, "a-level/physics/gravitational-fields/inverse-square-field-lab");

    {

      const setupButton = page.getByRole("button", { name: "Set up field lab" });
