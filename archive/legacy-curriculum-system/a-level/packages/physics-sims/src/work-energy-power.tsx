...
export const workEnergyPowerPackageId = "work-energy-power" as ConceptPackageId;
export const workEnergyPowerSimId = "energy-transfer-lab";
export type WorkEnergyPowerPredictionEvent = PredictionEvent;

export interface WorkEnergyPowerState {
...
export interface WorkEnergyPowerModel {
...
export const workEnergyPowerSpec: TSimulationSpec = {
  id: workEnergyPowerSimId,
    ...
    renderers: [
      {
        ...
        module: "@paideia/a-level-physics-sims/work-energy-power",
...
const defaultState: WorkEnergyPowerState = {
...
const presets: readonly {
  ...
  readonly state: WorkEnergyPowerState;
}[] = [
...
const currentState = (state: Partial<WorkEnergyPowerState>): WorkEnergyPowerState => ({
...
const signOfWork = (workJoules: number): WorkEnergyPowerModel["signDecision"] => {
  if (Math.abs(workJoules) < 1e-9) return "zero";
  return workJoules > 0 ? "positive" : "negative";
};

const transferLabel = (sign: WorkEnergyPowerModel["signDecision"]): string => {
...
export const workEnergyPowerModel = (
  state: WorkEnergyPowerState,
): KernelResult<WorkEnergyPowerModel> => {
...
export const EnergyTransferDiagram = ({
  ...
  readonly state: WorkEnergyPowerState;
  readonly model: WorkEnergyPowerModel;
}) => {
...
const ManipulateStage = () => {
  ...
  const { state, set } = useManipulate<WorkEnergyPowerState>();
  const current = currentState(state);
  const model = useMemo(() => workEnergyPowerModel(current), [current]);
...
const ObserveStage = () => {
  const stage = useStage();
  const state = currentState(useSimState<Partial<WorkEnergyPowerState>>());
  const model = workEnergyPowerModel(state);
...
export const WorkEnergyPowerSim = () => (
  <SimRuntime spec={workEnergyPowerSpec} packageId={workEnergyPowerPackageId}>
    <StageSurface />
  </SimRuntime>
);

export default WorkEnergyPowerSim;
