...
const simHarnessId = (branch, manifest, simSpec) =>
  `${branch}/${manifest.subject}/${manifest.id}/${simSpec.id}`;
...
const renderKnowledgeGraph = (branch, manifests) => {
  ...
  for (const manifestPath of manifests) {
    ...
    const simBlock =
        ...
        : `[
      {
        ...
        harnessId: ${jsString(simHarnessId(branch, manifest, simSpec))},
  ...
  if (containers.length === 0) {
    throw new Error(`No containers found for ${branch}`);
  }
...
const renderSimHarnessRegistry = (targets) => {
  const imports = [];
  const entries = [];
  const ids = new Set();
  let importIndex = 0;
  ...
  for (const target of targets) {
    ...
    for (const manifestPath of manifests) {
      ...
      const manifest = readYaml(manifestPath);
      const simSpecPath = manifest.simulation?.spec ? join(containerDir, manifest.simulation.spec) : null;
      if (simSpecPath === null) continue;

      const simSpec = readOptionalYaml(simSpecPath);
      if (simSpec === null) continue;

      const importName = toIdentifier(
        `${target.branch}-${manifest.subject}-${manifest.id}-${simSpec.id}`,
        importIndex,
      );
      importIndex += 1;
      imports.push(`import ${importName} from ${jsString(firstRendererModule(simSpec, manifestPath))};`);

      const id = simHarnessId(target.branch, manifest, simSpec);
      if (ids.has(id)) {
        throw new Error(`Duplicate generated sim harness id: ${id}`);
      }
      ids.add(id);

      entries.push(`  ${jsString(id)}: {
    id: ${jsString(id)},
    title: ${jsString(simSpec.title)},
    Component: ${importName},
  }`);
...
if (isDirectory(SIM_HARNESS_TARGET.appDir)) {
  ...
  process.stdout.write(
    `generate-knowledge-graph: wrote ${relative(REPO_ROOT, SIM_HARNESS_TARGET.output)}\n`,
  );
}
