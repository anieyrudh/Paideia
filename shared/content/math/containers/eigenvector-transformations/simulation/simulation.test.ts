...
test.describe("Eigenvector Transformations", () => {
  test.beforeEach(async ({ page }) => {
    await page.goto("/?sim=shared/math/eigenvector-transformations/eigenvector-transformations");
  });
  ...
  test("prediction-gate blocks invariant-direction evidence until commit", async ({ page }) => {
    ...
    await expect(page.getByLabel("Formula", { exact: true })).toContainText("Av = lambda");
    await expect(page.getByLabel("Formula", { exact: true })).not.toContainText("\\color");
