...
branch: shared
level: Shared core
syllabus_ref: Shared mathematics / Linear algebra / Eigenvectors
prerequisites:
...
Imagine drawing several arrows from the origin, then applying the same matrix to
all of them. Most arrows swing to a new line. A few special arrows only get
longer, shorter, or flipped while staying on the same line. Those preserved
directions are eigenvectors.

## First-Principles Explanation

A 2x2 matrix moves every input vector by combining the moved basis vectors. Most directions tilt to a new line. An eigenvector is special because the combination lands back on the original line. That is why eigenvectors are useful across modelling, data, and dynamics: they identify axes where a coupled transformation becomes a simple one-dimensional scale.
...
Formally, an eigenvector is a non-zero vector whose direction is preserved by a
linear transformation. If a matrix `A` sends `v` to one scalar multiple of itself,
then
...
The vector `v` is the eigenvector and the number `lambda` is the eigenvalue. The
eigenvalue tells the scale factor along that direction: positive values stretch
without flipping, negative values flip through the origin, and values between 0
and 1 shrink.
