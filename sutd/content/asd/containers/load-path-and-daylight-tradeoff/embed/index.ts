...
export const createEmbed = (): ContainerEmbedApi => {
  ...
  return {
    async load(target: Element): Promise<void> {
      target.setAttribute("data-paideia-container", "load-path-and-daylight-tradeoff");
      mounted = true;
    },
    ...
    syncTheme(_theme: ContainerEmbedTheme): void {
      // Theme propagation is host-owned; this marker keeps the v1 embed contract explicit.
    },
    destroy(): void {
      mounted = false;
    },
