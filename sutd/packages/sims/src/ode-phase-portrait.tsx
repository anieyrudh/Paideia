...
export const odePhasePortraitPackageId = "sutd/smt/ode-phase-portrait" as ConceptPackageId;
...
const FormulaTrail = ({ evidence }: { readonly evidence: PhaseEvidence }) => {
  ...
  return (
    <section aria-label="Formula panel" style={panelStyle}>
      ...
      <p>
        Trace formula: T = a + d = {format(a)} + {format(d)} = {format(stability.trace)}.
      </p>
      <p>
        Determinant formula: D = ad - bc = ({format(a)})({format(d)}) - ({format(b)})(
        {format(c)}) = {format(stability.determinant)}.
      </p>
      <p>
        Discriminant formula: Delta = T^2 - 4D = {format(stability.trace)}^2 - 4(
        {format(stability.determinant)}) = {format(stability.discriminant)}.
      </p>
      <p>
        Eigenvalue formula: lambda = (T +/- sqrt(Delta)) / 2, giving {eigenvalueText(stability)}.
      </p>
      <p>
        Trajectory setup: RK4 samples z' = A z from z0 = ({format(state.initialX)},{" "}
        {format(state.initialY)}) with dt = {integrationDt} for {integrationSteps} steps; final
