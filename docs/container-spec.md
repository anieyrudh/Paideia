...
**Status:** Phase B · v1.1.0 policy update. Schema-breaking changes still require an ADR and a numbered migration.

A **container** (formally: ConceptPackage) is the unit of work, of authoring, and of student-facing delivery. The catalogue lists containers. The student launches a container. An agent scaffolds a container. Review tools read a container.

A container holds **one concept**, exposed through the supporting items it declares: concept card, sources, sims, decision matrix, misconceptions, transfer problems, and assessments. Predict -> Manipulate -> Observe -> Explain -> Transfer is a useful pedagogical pattern, but it is not a required visible UI layout. A container may express its learning flow as tabs, a lab notebook, a game loop, a story sequence, or another interaction model.
...
│       └── <sim-id>.test.ts     REQUIRED · MUST contain prediction-gate Playwright if predict is declared
├── transfer/                    OPTIONAL
│   └── <problem-id>.md
├── assessments/                 OPTIONAL
│   └── fsrs-cards.yaml
├── README.md                    REQUIRED · auto-generated descriptive doc
└── TECHNICAL.md                 REQUIRED · auto-generated technical doc + review notes
```
...
Every file follows a canonical template. The scaffolder produces these on `pnpm container:new`. Author edits content, never structure. UI and interaction design are intentionally flexible; the manifest records what pieces exist and the validator enforces that declared pieces connect coherently.
...
(Describe the actual learner flow. PMOE-T labels are allowed but not required.)
...
## 6. Transfer surface
Why is the transfer problem genuinely different from the first practice surface?
```
...
The React entry point. Imports any required gate, kernel(s), renderer(s), and local presentation components. Hot-reloadable.

See `core/docs-templates/sim-index.template.tsx`.

### 2.8 `sims/<sim-id>/<sim-id>.test.ts`

Playwright test. If the container or sim declares a predict step, this MUST include at least one test asserting the prediction gate blocks reveal until commit. CI fails the build if this assertion is absent for a declared predict path.
...
- Tests (file paths)
- How to run locally
- **Anieyrudh Filter pass** (P0/P1 items + resolution) — required only once the container reaches the configured review lifecycle threshold
- Iteration log (what the agent did, what was rejected)

The `daily-compliance-audit.yml` scheduled workflow scans containers and creates Issues when a container has reached the configured Filter lifecycle threshold but its `TECHNICAL.md` Filter section is missing or empty.
...
4. Each sim with a declared predict path has a `*.test.ts` containing the literal token `prediction-gate` somewhere in the file (lint-level proxy for the real Playwright assertion).
5. `concept-card.md` frontmatter parses against `ConceptCardFrontmatter`.
6. `TECHNICAL.md` has a non-empty section under `## Anieyrudh Filter pass` once the container reaches the configured Filter lifecycle threshold.
7. Every kernel listed in `kernel_deps` resolves to an existing `core/<module>/` directory.
8. Optional declared pieces are coherent:
   - `aid_types: [simulation]` requires `items.sims`, and non-empty `items.sims` requires `simulation`.
   - `aid_types: [misconception-audit]` requires `items.misconceptions` plus at least one `misconceptions[]` entry.
   - `aid_types: [transfer-problem]` requires `items.transfer_problems`, and every listed transfer problem requires `transfer/<id>.md`.
   - `predict_at: package-level` or `both` requires `package_predict`.
   - `predict_at: per-sim` or `both` requires every listed sim to declare `predict`.
9. No file in the container imports from another branch (`a-level/` cannot import `sutd/` and vice versa). This is enforced by `boundary-check.yml` via dependency-cruiser.
...
2. `content-only` — concept card + sources filled; optional misconceptions, transfer, sims, and assessments may still be absent.
3. `draft` — declared pieces implemented enough for iteration. Filter pass optional unless the manifest config raises its threshold.
...
6. `published` — advisor sign-off recorded; Filter P0 count is zero; live on production catalogue.
