...
| Prediction-gate tests too weak | Some tests committed a prediction before proving the observation was absent. | `pnpm container:quality` requires non-legacy tests to assert no `Observation unlocked` region before commit. |
...
- non-legacy prediction-gate tests prove the revealed observation is absent
  before commit and visible after commit.
