...
import { ProbabilityStatisticsSim } from "./probability-statistics.js";
...
const renderSim = async () => {
  ...
  await act(async () => {
    root?.render(createElement(ProbabilityStatisticsSim));
  });
};
...
export const runProbabilityStatisticsGateContract = () => {
  describe("probability statistics prediction-gate contract", () => {
    it("blocks distribution readouts until the prediction gate is committed", async () => {
      await renderSim();

      await click(buttonByText("Set up distribution"));
      await click(buttonByText("Reveal decision"));

      expect(document.querySelector("[aria-label='Observation unlocked']")).toBeNull();
      expect(document.body.textContent).not.toContain("Expected score");
      expect(document.body.textContent).not.toContain("z = (5.40");

      await click(controlByLabel("The expected score can stay close"));
      await change(
        controlByLabel("Rationale"),
        "The rare high outcome changes spread as well as the centre.",
      );
      await click(buttonByText("Commit prediction"));

      expect(document.querySelector("[aria-label='Observation unlocked']")).not.toBeNull();
      expect(document.body.textContent).toContain("Expected score");
      expect(document.body.textContent).toContain("Reject H0");
      expect(document.body.textContent).toContain("E(X)");
    });
    ...
    it("updates the decision when sample size changes before reveal", async () => {
      ...
      await click(buttonByText("Reveal decision"));
      await click(controlByLabel("The expected score can stay close"));
      await change(controlByLabel("Rationale"), "The standard error is wider for a smaller sample.");
      await click(buttonByText("Commit prediction"));

      expect(document.querySelector("[aria-label='Distribution readout']")?.textContent).toContain(
        "Do not reject H0",
      );
