...
import { QuantityMapLab } from "./quantity-map.js";
...
export const runQuantityMapGateContract = () => {
  describe("quantity-map prediction-gate contract", () => {
    it("blocks the observation until the prediction gate is committed", async () => {
      ...
      await click(controlByLabel("mass + length + time^-2"));
      await change(
        controlByLabel("Rationale"),
        "Force should include mass and acceleration, so time must appear twice in the denominator.",
      );
      await click(commitButton());

      expect(document.querySelector("[aria-label='Observation unlocked']")).toBeTruthy();
      expect(text()).toContain("Quantity dependency map");
      expect(text()).toContain("dimensionally consistent");
      expect(text()).toContain("M L T^-2");
    });

    it("updates the verdict when a learner tests an impossible equation", async () => {
      await renderSim();

      await click(controlByLabel("mass + length + time^-2"));
      await change(controlByLabel("Rationale"), "I expect force to have M L T^-2 dimensions.");
      await click(commitButton());
      await change(controlByLabel("Equation to test"), "4");

      expect(document.querySelector("[aria-label='Dimension verdict']")?.textContent).toContain(
        "dimension mismatch",
      );
      expect(document.querySelector("[aria-label='Formula reasoning']")?.textContent).toContain(
        "missing one division by time",
      );
    });
  });
};
