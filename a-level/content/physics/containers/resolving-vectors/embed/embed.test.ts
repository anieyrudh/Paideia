...
import { createResolvingVectorsEmbed } from "./index.js";

describe("resolving-vectors embed contract", () => {
  it("exposes lifecycle methods and isolates host state", async () => {
    const embed = createResolvingVectorsEmbed();
    const target = document.createElement("section");

    await embed.load(target);
    expect(target.getAttribute("data-paideia-container")).toBe("resolving-vectors");
