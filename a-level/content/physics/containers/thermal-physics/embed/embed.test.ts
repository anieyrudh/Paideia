...
import { createThermalPhysicsEmbed } from "./index.js";

describe("thermal-physics embed contract", () => {
  it("exposes lifecycle methods and isolates host state", async () => {
    const embed = createThermalPhysicsEmbed();
    const target = document.createElement("section");

    await embed.load(target);
    expect(target.getAttribute("data-paideia-container")).toBe("thermal-physics");
