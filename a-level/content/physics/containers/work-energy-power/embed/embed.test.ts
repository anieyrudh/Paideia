...
import { createWorkEnergyPowerEmbed } from "./index.js";

describe("work-energy-power embed contract", () => {
  it("exposes lifecycle methods and isolates host state", async () => {
    const embed = createWorkEnergyPowerEmbed();
    const target = document.createElement("section");

    await embed.load(target);
    expect(target.getAttribute("data-paideia-container")).toBe("work-energy-power");
